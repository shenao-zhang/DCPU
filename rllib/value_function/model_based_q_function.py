...
class ModelBasedQFunction(AbstractQFunction):
    ...
    def forward(self, state, sim_q, action=torch.tensor(float("nan")), head=0):
        ...
        """
    #    unfreeze_parameters(self.policy)  #TODO
        with DisableGradient(
            self.sim.dynamical_model, self.sim.reward_model, self.sim.termination_model, sim_q
        ):
            sim_observation = self.sim.simulate(state, sim_q, head, action)
