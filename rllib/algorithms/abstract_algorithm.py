...
class AbstractAlgorithm(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.q_updating = deep_copy_module(self.policy)
        ...
        if policy is None:
            ...
            self.pathwise_loss = PathwiseLoss(critic=self.critic,policy=self.policy, q_pol=self.q_updating)
    ...
    def get_reward(self, observation):
        """Get Reward."""
        _, _, entropy = self.get_kl_entropy(observation, ref_policy=True)
    ...
    def critic_loss(self, observation, ref_policy):
        """Get critic loss.
    ...
    def regularization_loss(self, observation, ref_policy, num_trajectories=1):
        """Compute regularization loss."""
        kl_mean, kl_var, entropy = self.get_kl_entropy(observation, ref_policy)
        if not ref_policy:
    ...
    def get_kl_entropy(self, observation, ref_policy):
        ...
        if not ref_policy:
            if isinstance(observation, Observation):
                ...
                states = observation[0].state
            else:
                raise NotImplementedError
            pi_old = tensor_to_distribution(
                self.old_policy(states), **self.policy.dist_params
            )
            pi = tensor_to_distribution(self.q_updating(states), **self.policy.dist_params)
            try:
                action = pi.rsample()
            ...
            q_updating = self.q_updating
            q_dist = tensor_to_distribution(q_updating(state), **self.policy.dist_params)
            try:
                action = pi.rsample()
            except NotImplementedError:
                action = pi.sample()
            if not self.policy.discrete_action:
                action = self.policy.action_scale * (action.clamp(-1.0, 1.0))
            entropy, log_p = get_entropy_and_log_p(pi, action, self.policy.action_scale)
            _, log_q = get_entropy_and_log_p(q_dist, action, self.policy.action_scale)
            kl_mean, kl_var = separated_kl(p=q_dist, q=pi, log_p=log_q, log_q=log_p)
            return kl_mean, kl_var, entropy
    ...
    def update(self):
     ...
     #   self.critic.entropy_regularization = self.entropy_loss.eta.item()  # TODO
