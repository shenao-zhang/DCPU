...
#        print("optimizer pi")
        ...
        update_parameters(self.policy, self.q_updating, 0.0)

        for _ in tqdm(range(self.num_iter), disable=not self._training_verbose):
        #    print(i)
            cm = DisableGradient(self.q_updating)
            with cm:
                losses = self.optimizer_pi.step(closure=closure2)
